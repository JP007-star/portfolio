        ...
        <div className="max-w-4xl mx-auto">
          <div className="relative">
            ...
            {experience.map((exp, index) => (
              ...
              >
                <div
                  className={`absolute top-2 w-2.5 h-2.5 bg-indigo-500 rounded-full shadow-lg shadow-indigo-500/50 ${
                    index % 2 === 0
                ...
                <div
                  className={`ml-8 md:ml-0 glass-card rounded-2xl p-6 hover:border-indigo-500/30 transition-all ${
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {exp.achievements.map((achievement) => (
                      <span
                        key={achievement}
                        className="px-3 py-1 bg-indigo-500/10 text-indigo-300 text-xs rounded-full border border-indigo-500/20"
                      >
                        {achievement}
                      </span>
                    ))}
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
            {education.map((edu, index) => (
              <motion.div
                ...
                className="glass-card rounded-xl p-6 border border-zinc-800 hover:border-zinc-700 transition-all"
              >
                <span className="text-indigo-400 font-mono text-xs">{edu.period}</span>
                <h4 className="text-lg font-medium text-white mt-2 mb-1">{edu.degree}</h4>
                <p className="text-zinc-500 text-sm">{edu.institution}</p>
                <p className="text-zinc-600 text-sm mt-2">{edu.details}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
